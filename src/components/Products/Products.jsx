...
function Products({ productsFromApi, setProductsFromApi , showFilter}) {
  ...
  useEffect(() => {
    if (selectedCategory === 'All') {
      ...
      setProductsFromApi(fullProductList.filter(product => product.category === selectedCategory));
    }
          ...
          {categoriesArray.length > 0 ? (
            categoriesArray.map((category, index) => (
              <CategoryButton
                category={category}
                key={index}
                isSelected={selectedCategory === category} // Pass selected status
                onSelect={() => setSelectedCategory(category)} // Update selected category
              />
            ))
          ) : (
